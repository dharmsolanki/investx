...
@section('content')
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="value" style="color:{{ $pendingWithdrawals > 0 ? '#FBB924' : 'var(--text)' }}">
                {{ $pendingWithdrawals }}</div>
    ...
    <div class="grid-2" style="margin-bottom:2rem">
        ...
        <div class="card">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.2rem">
                ...
                <a href="{{ route('investments.my') }}" style="font-size:0.8rem;color:var(--gold);text-decoration:none">Sab
                    dekho →</a>
            </div>
            ...
            @forelse($investments as $inv)
                <div style="padding:0.8rem 0;border-bottom:1px solid rgba(255,255,255,0.04)">
                    <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:0.5rem">
                        ...
                        <div style="text-align:right">
                            <div style="font-size:0.9rem;font-weight:600;color:var(--gold)">
                                ₹{{ number_format($inv->principal_amount, 0) }}</div>
                    ...
                    @if ($inv->status === 'active')
                        ...
                        <div style="font-size:0.7rem;color:var(--muted);margin-top:3px">{{ $inv->progressPercent() }}%
                            complete • {{ $inv->daysRemaining() }} days left</div>
                    @endif
                </div>
            @empty
        ...
        <div class="card">
            ...
            @forelse($transactions as $txn)
                <div
                    style="display:flex;justify-content:space-between;align-items:center;padding:0.65rem 0;border-bottom:1px solid rgba(255,255,255,0.04)">
                    <div>
                        <span class="badge {{ $txn->type }}">{{ ucfirst($txn->type) }}</span>
                        <div style="font-size:0.72rem;color:var(--muted);margin-top:3px">
                            {{ $txn->created_at->format('d M Y, h:i A') }}</div>
                    </div>
                    <div
                        style="font-weight:600;font-size:0.9rem;color:{{ in_array($txn->type, ['deposit', 'profit', 'referral_bonus']) ? 'var(--green)' : 'var(--red)' }}">
    ...
    @if (!Auth::user()->reviews()->exists())
        <div class="card" style="margin-top:1.5rem">
            <div class="card-title">⭐ Apna Experience Share Karein</div>
            <form action="{{ route('review.store') }}" method="POST">
                @csrf
                <div class="form-group">
                    <label class="form-label">Rating</label>
                    <div style="display:flex;gap:0.5rem;font-size:1.8rem;margin-bottom:0.5rem" id="star-container">
                        @for ($i = 1; $i <= 5; $i++)
                            <span onclick="setRating({{ $i }})" style="cursor:pointer;color:var(--muted)"
                                data-star="{{ $i }}">★</span>
                        @endfor
                    </div>
    ...
    <script>
        function setRating(val) {
            ...
            document.querySelectorAll('#star-container span').forEach((s, i) => {
                s.style.color = i < val ? 'var(--gold)' : 'var(--muted)';
            });
        }
