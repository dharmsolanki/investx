...
class AdminDashboardController extends Controller
{
    public function index()
    {
        ...
        $pendingWithdrawals = Withdrawal::with('user', 'investment.plan')->where('status', 'pending')->latest()->take(5)->get();
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $monthlyData[] = [
                'month'      => $m->format('M'),
                'invested'   => (float) Transaction::where('type','deposit')->where('status','completed')->whereYear('created_at',$m->year)->whereMonth('created_at',$m->month)->sum('amount'),
                'commission' => (float) Transaction::where('type','commission')->where('status','completed')->whereYear('created_at',$m->year)->whereMonth('created_at',$m->month)->sum('amount'),
            ];
        }
